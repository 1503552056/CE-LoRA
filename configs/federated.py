...
class FederatedTrainingArguments:
    ...
    personalized_c_agg: bool = True

    # FederatedTrainingArguments（示例）
    data_sim_auto: bool = True               # 是否在客户端启动时自动生成 GMM
    data_sim_root: str = ""                  # 默认空则用 <checkpoint_dir>/datasim
    data_sim_overwrite: bool = False         # 存在则跳过
    data_sim_gmm_components: int = 3
    data_sim_gmm_covariance_type: str = "diag"   # ["diag","full","tied","spherical"]
    data_sim_gmm_reg_covar: float = 1e-5
    data_sim_gmm_max_iter: int = 200
    data_sim_gmm_tol: float = 1e-3
    data_sim_pca_dim: int = 0                # 0 表示不做 PCA
    data_sim_pca_whiten: bool = False
